...
if not os.path.exists(output_filepath):
    print("Writing truthcat to " + output_filepath)
    
    # Write the cat data to a FITS file
    cat.write(output_filepath, format="fits", overwrite=True)
else:
    print("Appending truthcat data to " + output_filepath)
    ...
    with fits.open(output_filepath, mode='update') as existing_cat:        
        ...
        # Get the new data (assuming `cat` has the same structure as existing_data)
        new_data = cat.as_array()
        
        # Standardize the data types of new_data to match existing_data
        ...
        # Update the data in the FITS file
